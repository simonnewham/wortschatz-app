...
export function Logo(props: { textSize?: string; logoSize?: number }) {
    return (
        <View className='flex-row items-center'>
            ...
            <Text className={`text-primary text-${props.textSize ?? '4xl'} font-semibold tracking-tighter`}>
                Wort
            </Text>
            <Text className={`text-[#d4fd52] text-${props.textSize ?? '4xl'} font-bold tracking-tighter`}>
                ki
            </Text>
            <Text className={`text-primary text-${props.textSize ?? '4xl'} tracking-tighter`}>
                ste
            </Text>
        </View>
    );
